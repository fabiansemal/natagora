...
class res_partner(models.Model):
	...
	address_history_ids =  fields.One2many('res.partner.address.history', 'partner_id', "Historique des adresses")
...
class res_partner_address_history(models.Model):
	...
	date_move = fields.Date('Date de déménagement', default=fields.Date.today)
	user_id = fields.Many2one('res.users', 'Utilisateur', select=True)
	partner_id = fields.Many2one('res.partner', 'Contact', select=True)
	name = fields.Char('Nom', len=128)
	ref = fields.Char('Code', len=32)
	street = fields.Char('Rue', len=64)
	street2 = fields.Char('Rue(2)', len=64)
	zip = fields.Char('Code postal', len=64)
	city = fields.Char('Commnune', len=64)
	state = fields.Char('Province', len=64)
	country_id = fields.Many2one('res.country', 'Pays', select=True)
