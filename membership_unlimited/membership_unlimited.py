...
class product_template(osv.osv):
    ...
    _columns = {
        'membership_product': fields.boolean('Adhésion'),
        'membership_combi': fields.boolean('Combi Adhésion'),
		'membership_combi_ids': fields.many2many('product.template', 'membership_combi_product_rel', 'parent_product_id', 'child_product_id', 'pprodt-combi'),
    }
...
class account_journal(osv.osv):
    ...
    _columns = {
        'membership_journal': fields.boolean('Journal Adhésion'),
    }
...
class membership_cancel_reason(osv.osv):
    ...
    _columns = {
        'name': fields.char('Nom', len=64, select=True),
...
class membership_partner_account(osv.osv):
    ...
    _columns = {
		'partner_id': fields.many2one('res.partner', 'Contact', select=True),
        'membership_amount': fields.float('Montant Adhésion'),
        'membership_start': fields.date("Date de début d'adhésion"),
        'membership_end': fields.date("Date de fin d'adhésion"),
        'membership_cancel': fields.date("Date d'annulation de l'adhésion"),
        'cancel_reason_id': fields.many2one('membership.cancel.reason', "Raison de l'annulation", select=True),
        'interval_type': fields.selection((('y','Jaar'),('m','Maand'),('w','Week'),('d','Dag')), "Typ d'intervale"),
        'interval_number': fields.integer("Nombre d'intervale"),
        'last_invoice_date': fields.date("Date de dernière facturation"),
        'next_invoice_date': fields.date("Date de la prochaine facturation"),
        'product_id': fields.many2one('product.product', 'Article', select=True),
        'state': fields.function(_state,
                        string="Statut de l'adhésion", type='selection',
        ...
        'nbr_inv_year': fields.selection((('1','1'),('2','2'),('3','3'),('4','4'),('6','6'),('12','12')),'Nombre de factures par années'),
    }
...
class membership_invoice_line(osv.osv):
    ...
    _columns = {
		'partner_id': fields.many2one('res.partner', 'Contact', select=True),
		'invoice_id': fields.many2one('account.invoice', 'Facture', select=True),
		'membership_id': fields.many2one('membership.partner.account', 'Adhésion', select=True),
	    'date_invoice': fields.date('Date Facture'),
	    'amount_total': fields.float('Montant'),
        'product_id': fields.many2one('product.product', 'Article', select=True),
        'invoice_state': fields.related('invoice_id', 'state', type='selection', relation='account.invoice', selection=INVOICE_STATE_SELECTION, string='Statut de la facture'),
    }
...
class account_invoice(osv.osv):
    ...
    _columns = {
        'membership_invoice': fields.boolean(' Facture Adhésion'),
        'membership_id': fields.many2one('membership.partner.account', 'Adhésion', select=True),
    }
...
class res_partner(osv.osv):
    ...
    _columns = {
        'membership_ids': fields.one2many('membership.partner.account', 'partner_id', 'Adhésions'),
        'membership_line_ids': fields.one2many('membership.invoice.line', 'partner_id', 'Fatures Adhésions'),
        'membership_state': fields.function(
                    __get_membership_state,
                    string = "Status d'adhésion", type = 'selection',
                    selection = STATE,
                    store = {
        ...
        'membership_start': fields.function(
                    ...
                    string = "Date début d'adhésion", type = 'date',
                    store = {
                        'account.invoice': (_get_invoice_partner, ['state'], 10),
                        'membership.partner.account': (_get_partner_id, ['state','membership_end', 'membership_cancel', 'membership_start'], 10, ),
        ...
        'membership_stop': fields.function(
                    _membership_date,
                    string = "Date fin d'adhésion", type='date', multi='membership_stop',
                    store = {
                        'account.invoice': (_get_invoice_partner, ['state'], 10),
                        'membership.partner.account': (_get_partner_id, ['state','membership_end', 'membership_cancel', 'membership_start'], 10, ),
                        'res.partner': (lambda self, cr, uid, ids, c={}: ids, ['free_member'], 10)
        ...
        'membership_cancel': fields.function(
                    _membership_date,
                    string = "Date d'annulation d'adhésion", type='date', multi='membership_cancel',
                    store = {
