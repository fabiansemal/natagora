...
class account_bank_statement_line(osv.osv):
	...
	_columns = {
		'analytic_account_id': fields.many2one('account.analytic.account', 'Code Analytique', select=True),
		'membership_invoice_id': fields.many2one('account.invoice', "Facture Adhésion", select=True),
	}
...
'''
	...
	_columns = {
		...
		'middle_name': fields.char('Particule'),
		'initials': fields.char('Initiales'),
		'street': fields.char('Rue'),
		'zip': fields.char('Code postal', size=16),
		'city': fields.char('Commune'),
		'bic': fields.char('Code BIC', size=16),
		'bank_account': fields.char('Compte banquaire', size=20),
		'transaction_amount': fields.float('Montant de la transaction'),
		'free_comm': fields.char('Communication libre'),
		'partner_id': fields.many2one('res.partner', 'Contact', select=True),
		'add_bank_account': fields.boolean('Ajouter un compte en banque'),
		'membership_state': fields.char("Statut d'adhésion", translate=True),
		'partner_address': fields.char('Adresse'),
		'accept_address': fields.boolean('Adresse acceptée'),
		'double_address': fields.text('Adresse doublon'),
		'stmt_id': fields.many2one('account.bank.statement.line', "Ligne d'extait de compte", select=True),
		'bank_account_id': fields.many2one('res.partner.bank', 'Compte en banque', select=True),
		'membership_partner': fields.boolean('Adhésion'),
		'donation_partner': fields.boolean('Donation'),
		'analytic_account_id': fields.many2one('account.analytic.account', 'Code analytique', select=True),
		'membership_invoice_id': fields.many2one('account.invoice', "Facture d'adhésion", select=True),
	}
