...
class res_partner(models.Model):
	...
	relation_partner_id = fields.Many2one('res.partner', 'Partenaire Conjugal', select=True)
	relation_partner_set = fields.Boolean('Relation', default=False)
	relation_address_set = fields.Boolean('Adresse Relation', default=False)
	birthday = fields.Date('Date de naissance')
	deceased = fields.Boolean('Décédé', default=False)
	deceased_partner_id = fields.Many2one('res.partner', 'transférer à', select=True)
	country_person_id = fields.Char('Numéro National')
	gender = fields.Selection([('m','Homme'),('f','Femme'),('u','Inconnu')], string='Genre', size=1)
