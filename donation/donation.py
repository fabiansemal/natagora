...
class product_template(osv.osv):
    ...
    _columns = {
        'donation_product': fields.boolean('Don'),
    }
...
class account_journal(osv.osv):
    ...
    _columns = {
        'donation_journal': fields.boolean('Journal Dons'),
    }
...
class donation_partner_account(osv.osv):
    ...
    _columns = {
        ...
        'analytic_account_id': fields.many2one('account.analytic.account', 'Compte analytique', select=True),
        'donation_amount': fields.float('Montant du don'),
        'donation_start': fields.date("Date de début de don"),
        'donation_end': fields.date("Date de fin de don"),
        'donation_cancel': fields.date("Date d'annulation du don"),
        'cancel_reason_id': fields.many2one('donation.cancel.reason', "Raison de l'annulation", select=True),
        'interval_type': fields.selection((('y','Jaar'),('m','Maand'),('w','Week'),('d','Dag')), "Type d'intervale"),
        'interval_number': fields.integer("Nombre d'intervale"),
        'last_invoice_date': fields.date('Date de dernière facturation'),
        'next_invoice_date': fields.date('Date de prochaine facturation'),
...
class donation_donation_line(osv.osv):
    ...
    _columns = {
		...
		'donation_id': fields.many2one('donation.partner.account', 'Don', select=True),
...
class account_invoice(osv.osv):
    ...
    _columns = {
        'donation_invoice': fields.boolean('Facture de don'),
        'donation_id': fields.many2one('donation.partner.account', 'Don', select=True),
    }
...
class res_partner(osv.osv):
    ...
    _columns = {
        'donation_ids': fields.one2many('donation.partner.account', 'partner_id', 'Dons'),
        'donation_line_ids': fields.one2many('donation.donation.line', 'partner_id', 'Factures de dons'),
    }
